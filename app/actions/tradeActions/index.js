  ...
  switch (currentStrategyCode) {
    case 1: {
      if (manualEthAmount > 0) {
        startManualEthHedging(
          ...
          current_bid,
          manualAggressionFactor,
          parseInt(manualEthAmount * Math.pow(10, 18))
        );
      }
      if (manualDaiAmount > 0) {
        startManualDaiHedging(
          ...
          current_bid,
          manualAggressionFactor,
          parseInt(manualDaiAmount * avgPrice * Math.pow(10, 18))
        );
      }
      break;
    }
    ...
    case 3: {
      startTradingBot(
        spreadPercentage,
        avgPrice,
        etherBalance + daiBalance * avgPrice
      );
      break;
    }
    default:
...
export const startManualEthHedging = (
  current_ask,
  current_bid,
  manualAggressionFactor,
  weis
  ...
  let hardPad = 5000;
  let softPad = -2000;
  ConversionRatesContract.methods
    .setImbalanceStepFunction(
      DAI_ADDRESS, //ERC20 token: KNC
      [
        web3.utils.toHex(parseInt(tradeSize / 10)),
        web3.utils.toHex(parseInt(2 * tradeSize))
      ],
...
export const startManualDaiHedging = (
  current_ask,
  current_bid,
  manualAggressionFactor,
  weis
  ...
  ConversionRatesContract.methods
    .setImbalanceStepFunction(
      DAI_ADDRESS, //ERC20 token: KNC
      [
        web3.utils.toHex(parseInt(tradeSize)),
        web3.utils.toHex(parseInt(2 * tradeSize))
      ],
...
export const startTradingBot = (
  ...
  var txData = await ConversionRatesContract.methods
    .setBaseRate(
      ...
      [
        web3.utils.toHex(
          parseInt((Math.pow(10, 18) / avgPrice) * (1 + percentage / 200))
        )
      ], //uint[] baseBuy
      [
        web3.utils.toHex(
...
export const balanceRatios = (
  ...
  let currentRatio =
    (etherBalance * 100) / (etherBalance + daiBalance * avgPrice);
  ...
  if (currentRatio > balanceRatio) {
    ...
    let tradeSize =
      ...
      ((etherBalance + daiBalance * avgPrice) / 100);
    let hardPad = 5000;
    let softPad = -2000;
    ConversionRatesContract.methods
      .setImbalanceStepFunction(
